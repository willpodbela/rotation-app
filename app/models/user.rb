class User < ApplicationRecord
  ...
  after_create do |user|
    ...
    # NOTE: Disabled for Heroku deployment - Redis not configured
    # CustomerFeedbackMailer.with(user: user).missed_conversion.deliver_later(wait_until: CustomerFeedbackMailer.preferred_time.advance(days: 5))
